...
#define SMBUS_PORT	GPIOE      //PB端口(端口和下面的两个针脚可自定义)
#define SMBUS_SCK		GPIO_Pin_11 //PE11：SCL
#define SMBUS_SDA		GPIO_Pin_15 //PE15：SDA

#define RCC_AHB1Periph_SMBUS_PORT		RCC_AHB1Periph_GPIOE

#define SMBUS_SCK_H()	    PEout(11)=1 //置高电平
#define SMBUS_SCK_L()	    PEout(11)=0  //置低电平
#define SMBUS_SDA_H()	    PEout(15)=1
#define SMBUS_SDA_L()	    PEout(15)=0

#define SMBUS_SDA_PIN()	   PEin(15) //读取引脚电平


/**************************************GXY*******************************************/
//IO方向设置
#define SMBUS_SDA_IN()  {GPIOE->MODER&=~(3<<(15*2));GPIOE->MODER|=0<<15*2;delay_us(1);}	//PE15输入模式
#define SMBUS_SDA_OUT() {GPIOE->MODER&=~(3<<(15*2));GPIOE->MODER|=1<<15*2;delay_us(1);} //PE15输出模式
...
void SMBus_Init()
{
		...
		GPIO_InitStructure.GPIO_Pin = SMBUS_SCK | SMBUS_SDA; //|GPIO_Pin_7 | GPIO_Pin_9;
...
{   
	float temp;
	temp = SMBus_ReadMemory(SA, RAM_ACCESS|RAM_TOBJ1)*0.02-273.15+1;
	return temp;
}
